...
def run():
    ...
    parser.add_argument(
        ...
        help="Directory with the result data from the output " "space toy experiment.",
    )
    ...
    for i, key in enumerate(methods):
        ax.arrow(
            output_arrows_start[key][0],
            output_arrows_start[key][1],
            output_arrows[key][0],
            output_arrows[key][1],
            width=0.08,
            head_width=0.4,
    ...
    # dimensions
    output_start = y
    distance = np.linalg.norm(output_start.detach().cpu().numpy() - target.detach().cpu().numpy())
