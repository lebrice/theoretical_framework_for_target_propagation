...
def train(args, device, train_loader, net, writer, test_loader, summary, val_loader):
    ...
    train_var.forward_optimizer, train_var.feedback_optimizer = utils.choose_optimizer(args, net)
    if args.classification:
        if args.output_activation == "softmax":
            ...
            raise ValueError(
                "The mnist dataset can only be combined with a "
    ...
    if args.epochs_fb == 0 or args.freeze_fb_weights:
        ...
        for e_fb in range(args.epochs_fb):
            ...
            train_only_feedback_parameters(args, train_var, device, train_loader, net, writer)
            av_reconstruction_loss_init = np.mean(train_var.reconstruction_losses_init)
            if e_fb == 0:
                train_var.summary["rec_loss_first"] = av_reconstruction_loss_init

            print(
                "init epoch {}, reconstruction loss: {}".format(
        ...
        train_var.summary["rec_loss_init_combined"] = 0.5 * (
            train_var.summary["rec_loss_init"] + train_var.summary["rec_loss_first"]
        )
    ...
    if args.output_space_plot:
        ...
        if args.classification:
            if args.output_activation == "sigmoid":
                targets = utils.int_to_one_hot(targets, 10, device, soft_target=1.0)
            else:
    ...
    for e in range(args.epochs):
        ...
        if not args.freeze_fb_weights:
            for extra_e in range(args.extra_fb_epochs):
                train_only_feedback_parameters(
                    args, train_var, device, train_loader, net, writer, log=False
                )

        train_var.test_accuracy, train_var.test_loss = test(
            args, device, net, test_loader, train_var.loss_function
        )
        if not args.no_val_set:
            train_var.val_accuracy, train_var.val_loss = test(
                args, device, net, val_loader, train_var.loss_function
            )

        # print intermediate results
        train_var.epoch_loss = np.mean(train_var.losses)
        print("Epoch {} -- training loss = {}.".format(e + 1, train_var.epoch_loss))
        if not args.no_val_set:
        ...
        if args.classification:
            ...
            print("Epoch {} -- training acc  = {}%".format(e + 1, train_var.epoch_accuracy * 100))
            if not args.no_val_set:
                print("Epoch {} -- val acc  = {}%".format(e + 1, train_var.val_accuracy * 100))
            print("Epoch {} -- test acc  = {}%".format(e + 1, train_var.test_accuracy * 100))
        else:
        ...
        train_var.epoch_losses = np.append(train_var.epoch_losses, train_var.epoch_loss)
        train_var.test_losses = np.append(train_var.test_losses, train_var.test_loss)

        if not args.no_val_set:
            train_var.val_losses = np.append(train_var.val_losses, train_var.val_loss)
        if not args.freeze_fb_weights:
            ...
            train_var.epoch_reconstruction_losses = np.append(
                train_var.epoch_reconstruction_losses, av_epoch_reconstruction_loss
            )
            train_var.epoch_reconstruction_losses_var = np.append(
                train_var.epoch_reconstruction_losses_var, var_epoch_reconstruction_loss
            )

        if args.classification:
            train_var.epoch_accuracies = np.append(
                train_var.epoch_accuracies, train_var.epoch_accuracy
            )
            train_var.test_accuracies = np.append(
                train_var.test_accuracies, train_var.test_accuracy
            )
            if not args.no_val_set:
                train_var.val_accuracies = np.append(
                    train_var.val_accuracies, train_var.val_accuracy
                )
        ...
        if e == 2:
            if args.gn_damping_hpsearch:
                print(
                    "Doing hpsearch for finding ideal GN damping constant"
                    "for computing the angle with GNT updates"
                )
                gn_damping = gn_damping_hpsearch(args, train_var, device, train_loader, net, writer)
    ...
    if not args.epochs == 0:
        ...
        if not args.no_val_set:
            ...
            train_var.summary["loss_test_val_best"] = train_var.test_losses[best_epoch]
            train_var.summary["loss_train_val_best"] = train_var.epoch_losses[best_epoch]

        if not args.freeze_fb_weights:
            train_var.summary["rec_loss_last"] = av_epoch_reconstruction_loss
            train_var.summary["rec_loss_best"] = train_var.epoch_reconstruction_losses.min()
        ...
        if args.classification:
            ...
            train_var.summary["acc_train_growth"] = (
                train_var.epoch_accuracies[-1] - train_var.epoch_accuracies[0]
            )
            train_var.summary["acc_test_growth"] = (
                train_var.test_accuracies[-1] - train_var.epoch_accuracies[0]
            )
            ...
            if not args.no_val_set:
                ...
                train_var.summary["acc_test_val_best"] = train_var.test_accuracies[best_epoch]
                train_var.summary["acc_train_val_best"] = train_var.epoch_accuracies[best_epoch]
...
def train_parallel(args, train_var, device, train_loader, net, writer):
    ...
    for i, (inputs, targets) in enumerate(train_loader):
        ...
        if args.classification and args.output_activation == "sigmoid":
            ...
            targets = utils.int_to_one_hot(targets, 10, device, soft_target=args.soft_target)

        train_var.batch_accuracy, train_var.batch_loss = train_forward_parameters(
            args, net, predictions, targets, train_var.loss_function, train_var.forward_optimizer,
        )
        if not args.freeze_fb_weights:
            train_feedback_parameters(args, net, train_var.feedback_optimizer)

        if args.classification:
            train_var.accuracies = np.append(train_var.accuracies, train_var.batch_accuracy)
        train_var.losses = np.append(train_var.losses, train_var.batch_loss.item())
        if not args.freeze_fb_weights:
            train_var.reconstruction_losses = np.append(
                train_var.reconstruction_losses, net.get_av_reconstruction_loss()
            )
        ...
        if args.save_logs and i % args.log_interval == 0:
            if not args.freeze_fb_weights:
                utils.save_feedback_batch_logs(args, writer, train_var.batch_idx, net)
            utils.save_forward_batch_logs(
                args, writer, train_var.batch_idx, net, train_var.batch_loss, predictions,
            )
            train_var.batch_idx += 1

        if not args.freeze_fb_weights and args.extra_fb_minibatches > 0:
            train_extra_fb_minibatches(args, train_var, device, train_loader, net)
...
def train_separate(args, train_var, device, train_loader, net, writer):
    ...
    if not args.freeze_fb_weights:
        for i, (inputs, targets) in enumerate(train_loader):
            ...
            if args.save_logs and i % args.log_interval == 0:
                utils.save_feedback_batch_logs(args, writer, train_var.batch_idx_fb, net)
...
def train_forward_parameters(args, net, predictions, targets, loss_function, forward_optimizer):
    ...
    if not args.train_randomized:
        net.backward(
            loss, args.target_stepsize, save_target=save_target, norm_ratio=args.norm_ratio,
        )
    else:
        k = np.random.randint(0, net.depth)
        net.backward_random(
            loss, args.target_stepsize, k, save_target=save_target, norm_ratio=args.norm_ratio,
        )
...
def test(args, device, net, test_loader, loss_function):
    ...
    with torch.no_grad():
        for inputs, targets in test_loader:
            ...
            if args.classification:
                if args.output_activation == "sigmoid":
                    accuracy += utils.accuracy(predictions, utils.one_hot_to_int(targets))
                else:  # softmax
...
def train_only_feedback_parameters(args, train_var, device, train_loader, net, writer, log=True):
    ...
    for i, (inputs, targets) in enumerate(train_loader):
        ...
        for l, layer in enumerate(net.layers):
            ...
            if loss_rec is not None and args.plots is not None and log:
                net.reconstruction_loss_init.at[train_var.epochs_init, l] = loss_rec
        if log:
            train_var.reconstruction_losses_init = np.append(
                train_var.reconstruction_losses_init, net.get_av_reconstruction_loss()
            )
        if args.save_logs and i % args.log_interval == 0 and log:
            utils.save_feedback_batch_logs(args, writer, train_var.init_idx, net, init=True)
            train_var.init_idx += 1
...
def train_bp(args, device, train_loader, net, writer, test_loader, summary, val_loader):
    print("Training network ...")
    net.train()
    ...
    if args.output_space_plot:
        ...
        utils.make_plot_output_space_bp(
            args, net, args.output_space_plot_layer_idx, loss_function, targets, inputs, steps=20,
        )
        return summary
    ...
    for e in range(args.epochs):
        ...
        for i, (inputs, targets) in enumerate(train_loader):
            ...
            if args.classification:
                if args.output_activation == "sigmoid":
                    running_accuracy += utils.accuracy(predictions, utils.one_hot_to_int(targets))
                else:  # softmax
                    running_accuracy += utils.accuracy(predictions, targets)

        test_accuracy, test_loss = test_bp(args, device, net, test_loader, loss_function)
        if not args.no_val_set:
            val_accuracy, val_loss = test_bp(args, device, net, val_loader, loss_function)
        ...
        if args.classification:
            print("Epoch {} -- training acc  = {}%".format(e + 1, epoch_accuracy * 100))
            if not args.no_val_set:
                print("Epoch {} -- val acc  = {}%".format(e + 1, val_accuracy * 100))
            print("Epoch {} -- test acc  = {}%".format(e + 1, test_accuracy * 100))
        if args.save_logs:
            utils.save_logs(
                writer,
                step=e + 1,
    ...
    if not args.epochs == 0:
        ...
        if not args.no_val_set:
            ...
            summary["loss_test_val_best"] = test_losses[best_epoch]
            summary["loss_train_val_best"] = epoch_losses[best_epoch]
        ...
        if args.classification:
            ...
            if not args.no_val_set:
                ...
                summary["acc_test_val_best"] = test_accuracies[best_epoch]
                summary["acc_train_val_best"] = epoch_accuracies[best_epoch]
    utils.save_summary_dict(args, summary)
...
def gn_damping_hpsearch(args, train_var, device, train_loader, net, writer):
    ...
    damping_values = np.logspace(-5.0, 1.0, num=7, base=10.0)
    ...
    for k, gn_damping in enumerate(damping_values):
        ...
        for i, (inputs, targets) in enumerate(train_loader):
            ...
            # print(predictions.shape, targets.shape)
            acc, loss = train_forward_parameters(
                args,
                net,
                predictions,
                targets,
