...
class DDTPConvNetwork(nn.Module):
    def __init__(
        ...
        nb_feedback_iterations=[10, 20, 20, 30, 55, 20],
    ):
    ...
    @property
    def layers(self) -> list[DDTPConvLayer | DDTPMLPLayer]:
    ...
    def propagate_backward(self, output_target, i):
        """
        Propagate the output target backwards ~to~ (edit: FROM) layer i in a DTP-like fashion.
        Args:
        ...
        layer_target = self.layers[i].backward(output_target, layer_activation, output_activation)
        return layer_target

    def backward(self, loss: Tensor, target_lr: float, save_target=False, norm_ratio=1.0):
        output_target = self.compute_output_target(loss, target_lr)
        self.layers[-1].compute_forward_gradients(output_target, self.layers[-2].activations)
        if save_target:
        ...
        for i in range(self.depth - 1):
            ...
            if i == 0:
                self.layers[i].compute_forward_gradients(
                    h_target, self.input, self.forward_requires_grad
                )
            else:
                previous_activations: Tensor | None = self.layers[i - 1].activations
                if i == self.nb_conv:  # flatten conv layer
    ...
    def save_logs(self, writer, step):
        for i in range(len(self.layers)):
            ...
            self.layers[i].save_logs(writer, step, name, no_gradient=i == len(self.layers) - 1)

    def save_feedback_batch_logs(self, writer, step, init=False):
        for i in range(len(self.layers)):
            name = "layer {}".format(i + 1)
            self.layers[i].save_feedback_batch_logs(
                writer, step, name, no_gradient=i == len(self.layers) - 1, init=init
            )
    ...
    def compute_bp_angles_and_distance(self, loss, i, retain_graph=False):
        ...
        weights_distance = utils.compute_distance(bp_gradients[0].detach(), gradients[0], True)
        if self.layers[i].bias is not None:
            bias_angle = utils.compute_angle(bp_gradients[1].detach(), gradients[1])
            bias_distance = utils.compute_distance(bp_gradients[1].detach(), gradients[1], True)
    ...
    def compute_weight_angles_and_distance(self, i, retain_graph=False):
        ...
        weights_angle = utils.compute_angle(transpose_feedback_weights, forward_weights)
        weights_distance = utils.compute_distance(
            transpose_feedback_weights, forward_weights, False
        )
        # if self.layers[i].bias is not None:
    ...
    def save_bp_activation_angle(self, writer, step, loss, retain_graph=False):
        ...
        for i in layer_indices:
            ...
            if i != layer_indices[-1]:  # if it is not the last index, the graph
                ...
                retain_graph_flag = retain_graph
            angle, distance = self.compute_bp_activation_angle_and_distance(
                loss, i, retain_graph_flag
            )
    ...
    def compute_bp_activation_angle_and_distance(self, loss, i, retain_graph=False, linear=False):
        """
        ...
        distance = utils.compute_average_batch_distance(
            target_difference.detach(), bp_updates, True
        )

        return angle, distance

    def compute_gnt_angle_and_distance(
    ...
    def save_gnt_angles(
        ...
        for i in layer_indices:
            ...
            if i != layer_indices[-1]:  # if it is not the last index, the graph
                ...
                retain_graph_flag = retain_graph
            angles, distances = self.compute_gnt_angle_and_distance(
                output_activation=output_activation,
                loss=loss,
                damping=damping[i],
                i=i,
            ...
            if custom_result_df is not None:
                ...
                writer.add_scalar(
                    tag="{}/weight_gnt_angle".format(name), scalar_value=angles[0], global_step=step
                )
...
class DDTPConvNetworkCIFAR(DDTPConvNetwork):
    ...
    def __init__(
        ...
        l1 = DDTPConvLayer(
            ...
            (5, 5),
            output_size=10,
            feature_size=[32, 16, 16] if type(self).pool_padding == 1 else [32, 15, 15],
            stride=1,
            padding=2,
            dilation=1,
            groups=1,
            ...
            pool_kernel_size=(3, 3),
            pool_stride=(2, 2),
            pool_padding=type(self).pool_padding,
            pool_dilation=1,
            forward_activation=hidden_activation,
            feedback_activation=feedback_activation,
            nb_feedback_iterations=nb_feedback_iterations[0],
        ...
        l3 = DDTPMLPLayer(
            in_features=(8 * 8 * 64 if type(self).pool_padding == 1 else 7 * 7 * 64),
            out_features=512,
            size_output=10,
            bias=bias,
            forward_requires_grad=forward_requires_grad,
...
class BPConvNetwork(nn.Module):
    def __init__(self, bias=True, hidden_activation="tanh", initialization="xavier_normal"):
        super().__init__()
        l1 = nn.Conv2d(
            3,
            96,
