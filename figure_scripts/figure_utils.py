...
def create_layerwise_result_dict(result_key_dict):
    ...
    for name, dtframe in result_key_dict.items():
        ...
        for i in range(nb_layers):
            layerwise_dict[i] = pd.concat([layerwise_dict[i], dtframe[i]], axis=1)
...
def save_result_dict(result_dict, out_dir):
    ...
    for key, sub_dict in result_dict.items():
        keys.append(key)
        np.append(is_array, isinstance(sub_dict[list(sub_dict.keys())[0]], np.ndarray))
        ...
        for name in names:
            ...
            if not "angle" in file_name:
                ...
                if os.path.exists(file_name + ".csv"):
                    result_dict[key][name] = pd.read_csv(file_name + ".csv", index_col=0)
...
def plot_rolling(
    ...
    for name, df in result_key_dict.items():
        ...
        for layer in range(nlayers):
            df[f"roll_mean_{layer}"] = (
                df.iloc[:, layer].rolling(window=smooth, min_periods=1).mean()
            )
            df[f"roll_std_{layer}"] = df.iloc[:, layer].rolling(window=smooth, min_periods=1).std()
            df[f"lower{layer}"] = df[f"roll_mean_{layer}"].sub(
                df[f"roll_std_{layer}"], fill_value=0
            )
            df[f"upper{layer}"] = df[f"roll_mean_{layer}"].add(
                df[f"roll_std_{layer}"], fill_value=0
            )
    ...
    if len(experiments) >= 9:
        color_palette[8] = (0.0, 0.796875, 0.796875)
    ...
    for layer in range(nlayers):
        ax = sns.lineplot(
            x="batch_idx", y=f"roll_mean_{layer}", hue="experiment", data=df, estimator=None,
        )
        for i, experiment in enumerate(experiments):
            df_fill = df.loc[df["experiment"] == experiment]
            ax.fill_between(
                df_fill["batch_idx"],
                df_fill[f"lower{layer}"],
                df_fill[f"upper{layer}"],
                alpha=0.3,
                lw=0,
        ...
        ax.legend(handles=handles[1:], labels=labels[1:], ncol=2, bbox_to_anchor=(0.16, 0.69))
        ax.xaxis.set_major_locator(plt.MaxNLocator(4))
        ax.yaxis.set_major_locator(plt.MaxNLocator(5))
        sns.despine()
        if not no_title:
