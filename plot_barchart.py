...
def run():
    ...
    parser.add_argument(
        ...
        help="Directory with the result data from the nullspace" "experiment.",
    )
    ...
    result_dict_null_DTP.insert(
        result_dict_null_DTP.shape[1],
        "type",
        [legend[0] for i in range(result_dict_null_DTP.shape[0])],
    )

    result_dict_null_DDTPlin.insert(
        result_dict_null_DDTPlin.shape[1],
        "type",
        [legend[1] for i in range(result_dict_null_DDTPlin.shape[0])],
    )
    ...
    ax.set_xlabel("")
    ax.set_ylabel(
        r"$\|\|\Delta W_%i^{null}\|\|/\|\|\Delta W_%i\|\|$" % (layer_idx + 1, layer_idx + 1)
    )
    ax.yaxis.set_major_locator(plt.MaxNLocator(4))
    ax.tick_params(axis="x", length=0)
    sns.despine()
    plt.savefig(os.path.join(result_dir, "barplot_nullspace.pdf"), bbox_inches="tight", dpi=400)
